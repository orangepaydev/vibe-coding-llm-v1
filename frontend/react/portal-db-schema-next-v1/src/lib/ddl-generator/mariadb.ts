...
export class MariaDBGenerator implements DDLGenerator {
  generateDDL(schema: Schema): string {
    const ddl: string[] = [];
    
    // Add header comment
    ddl.push(`-- MariaDB DDL for schema: ${schema.schemaName}`);
  ...
  generateTableDDL(table: Table): string {
    ...
    // MariaDB specific: Use InnoDB with modern charset
  ...
  mapColumnType(column: Column): string {
    ...
    // MariaDB type mapping (very similar to MySQL with some enhancements)
    switch (type) {
      case 'INT':
      case 'INTEGER':
      ...
      case 'LONGBLOB':
        return 'LONGBLOB';
      case 'JSON':
        // MariaDB has native JSON support since 10.2
        return 'JSON';
      case 'UUID':
        // MariaDB 10.7+ has UUID type
        return 'UUID';
      default:
  ...
  generateColumnDefinition(column: Column): string {
    ...
    if (column.defaultValue) {
      ...
      if (defaultVal.toLowerCase().includes('current_timestamp')) {
        ...
        parts.push('DEFAULT CURRENT_TIMESTAMP');
      } else if (defaultVal.toLowerCase() === 'uuid()') {
